...
var create = module.exports = function(options){
	...
	if(options.adaptMapping) server.adaptMapping = options.adaptMapping;
...
HttpServer.prototype.getResource = function(request, response){
	...
	fs.exists(filePath, function(exists){
		if(exists){
            ...
            if (~filePath.indexOf(self.adaptMapping[0])) {
                serveAdaptedMappingFile(request, response, filePath, self.adaptMapping[1]);
            ...
            } else if(~self.autoAdaptExtensions.indexOf(path.extname(filePath))){
...
function serveAdaptedMappingFile(request, response, filePath, adaptName){
	var self = this;
	fs.readFile(filePath, function(err, data){
		...
		data = data.toString();
		// 匹配 head
		var match = data.match(/<\/head>/);
		if(!match) return;
		fs.readFile(adapters[adaptName], function(err, adapterScript){
			if(err){
				self.log("[HTTP Server] Error in reading adapter script: " + err);
			} else {
				var injectPosition = match.index;
